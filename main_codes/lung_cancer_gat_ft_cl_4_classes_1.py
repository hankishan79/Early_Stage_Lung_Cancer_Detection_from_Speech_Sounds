...
data = pd.read_csv(
    r'..\dataset\feature_dataset.csv')
data.head()

df = pd.DataFrame(data)
# 1. grup için filtreleme
group1_df = df[df['Stage'] == 1]
# 2. grup için filtreleme (2-4 arasındaki değerlere sahip satırları içerir)
group2_df = df[df['Stage'].between(2, 4)]
# Sonuçları yazdır
print("1. Grup DataFrame:")
print(group1_df)
print("\n2. Grup DataFrame:")
print(group2_df)
# Grup DataFrame'leri oluşturulduktan sonra:
...
y = combined_df['Stage']
X = combined_df.drop(columns=['CLASSES', 'Segment', 'Stage', 'PAT_ID'], axis=1)
#,'MFCC1', 'MFCC2', 'MFCC3', 'MFCC4', 'MFCC5', 'MFCC6','MFCC7', 'MFCC8', 'MFCC9', 'MFCC10', 'MFCC11', 'MFCC12','MFCC13','MIN','MAX','MEAN','RMS','VAR','STD','POWER','PEAK','P2P','CREST FACTOR','SKEW','KURTOSIS','MAX_f','SUM_f','MEAN_f','VAR_f','PEAK_f','SKEW_f','KURTOSIS_f'
#,'MIN','MAX','MEAN','RMS','VAR','STD','POWER','PEAK','P2P','CREST FACTOR','SKEW','KURTOSIS','MAX_f','SUM_f','MEAN_f','VAR_f','PEAK_f','SKEW_f','KURTOSIS_f','PCA1','PCA2','PCA3'
...
output_size_classifier = 4  # assuming 2 classes
...
with torch.no_grad():
    ...
    auc2 = roc_auc_score(y_test_np, predicted_labels_np, multi_class='ovo')
